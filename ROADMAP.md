...
| P9.27 | 러너: `repo_subfolder` 오탐 수정 — 다른 최상위 폴더에 코드가 있으면 하위 폴더 repo가 아님 (§6 2026-09-23 P9.27) | P9.26 | done | 494bae8 |
...
|---|---|---|---|
| 2026-09-21 | **결정 요청: 방문자 간 프로젝트 격리** | 사용자 지적 | "동시에 2명이 접속해도 서로의 프로젝트나 진행이 보이면 안 되는 것 아니냐". 현재: 인증 없음(2026-09-19 결정 "회원 기능 없음"), `GET /projects`가 전부 반환, 콘솔 사용자는 모두 `judge`, 데모 모드 OFF라 `DELETE /projects`·cancel도 누구나 가능(AdminDep는 데모 모드에서만). 옵션: (A) GitHub OAuth 로그인 — 로그인한 사람이 접근 가능한 repo의 프로젝트만 보이고 조작 가능. 정석이지만 App에 callback URL·client secret 설정(사용자 작업), 세션 구현 ~1일 (B) 프로젝트 키 — 연결 시 서버가 비밀 키를 발급해 그 브라우저(localStorage)에 저장, 목록·조회·조작·WS에 키 필요, 다른 기기는 키 포함 링크로 공유. 로그인 없이 "링크만" 규정에 맞음, ~0.5일 (C) 목록만 숨김 — 콘솔이 이 브라우저가 연결한 프로젝트만 표시, API는 그대로(보안 아님). 추천: (B) 지금, (A)는 P10 |
| 2026-09-23 | (기록) P9.27 `repo_subfolder` 오탐 | 사용자 지적 | "자꾸 goal이 취소되는 문제". 방문자 hjunhuh/homebrew-cask(Ruby tap) Goal 16개가 18:27~18:31 KST에 전부 `repo_subfolder: 'cmd/'`로 취소. 원인: 루트 표식에 Ruby가 없고(`.rb`는 `Casks/a/*.rb`처럼 2단계 아래), `cmd/find-appcast.rb` 한 파일이 유일한 '표식 있는 최상위 폴더'가 됨 — 바로 아래만 보는 규칙이라 `Casks/`의 수천 개 코드를 못 봄. 수정: 후보 폴더 밖의 다른 최상위 폴더(숨김·docs 제외)에 **어느 깊이든** 코드·manifest가 있으면 하위 폴더 repo가 아니다(P9.25 '오탐 방지' 취지 그대로). melpes 형태(코드가 폴더 하나에만)는 계속 감지 |
