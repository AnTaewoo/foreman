...
0. **Task 계약을 산문 spec에서 실행 가능한 인수 테스트로** (D-55, 제출 후 P10) — 세 번의 라이브 실패는 거의 전부
   Task 경계(소유 경로 밖 쓰기, import 의존 누락, 테스트 전용·설치 Task, 자리표시자, 옛 base)에서 났다. Plan의 AC를
   `tests/test_goal_<id>.py`로 만들어 사람이 승인하고, Task = "이 테스트를 통과시켜라 + 파일들"로 두면 이 실패들이
   규칙이 아니라 구조로 사라진다. 에이전트는 인수 테스트를 고칠 수 없고, Gate는 Task 테스트 + 전체 스위트.
