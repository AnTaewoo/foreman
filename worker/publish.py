"""워커 이벤트 발행: DB 없이 Redis ``events:{project_id}``에 XADD (D-26 — 서명·DB append는 Scheduler ingest).

스트림 필드는 ``control_plane.events.bus.stream_fields``와 같은 형식(id/type/seq/canonical/signature)이지만
워커는 sqlalchemy를 끌어오지 않도록 여기서 다시 만든다. ``FilePublisher``는 테스트/디버그용 JSON-lines.
"""
...
def stream_fields_for(event: Event) -> dict[str, str]:
    return {
        "id": event.id,
