...
컨테이너는 호스트의 Redis(docker compose, host-gateway)에 XADD 하고, 마운트된 로컬 bare remote를 clone·push 한다.
"""
...
async def test_docker_launcher_starts_worker(tmp_path: Path) -> None:
    ...
    subprocess.run(["docker", "build", "-q", "-t", "foreman-worker:test", "-f", "worker/Dockerfile", "."],
    ...
    task_json = {
        ...
        "project_context": {"project_id": "PDOCK", "goal_id": "G1", "repo": str(remote), "default_branch": "main"},
        "run_id": "01RUNDOCKER", "agent_id": "coding-1",
    }  # fmt: skip
    spec = LaunchSpec(task_id="T1", run_id="01RUNDOCKER", project_id="PDOCK", goal_id="G1",
    ...
    launcher = DockerCliLauncher(
        ...
        mounts=[(str(tmp_path), str(tmp_path)), (str(FIXTURES / "coding_scripts"), "/scripts")],
    )
